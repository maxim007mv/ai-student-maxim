...
def test_new_quality_flags():
    # Создаем тестовый DataFrame с новыми эвристиками
    df = pd.DataFrame({
        'user_id': [1, 2, 2, 4, 5],  # дубликат ID
        'category': ['A'] * 5,  # константная колонка
        'value': [0, 0, 0, 0, 1],  # много нулей
        'high_card': ['A', 'B', 'C', 'D', 'E']  # высокая кардинальность (для 5 строк)
    })
    ...
    assert 'value' in flags['zero_heavy_columns']
