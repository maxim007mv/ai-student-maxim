...
def report(
    path: str = typer.Argument(..., help="Путь к CSV-файлу."),
    out_dir: str = typer.Option("hw03_report", help="Каталог для отчёта."),
    sep: str = typer.Option(",", help="Разделитель в CSV."),
    encoding: str = typer.Option("utf-8", help="Кодировка файла."),
    ...
    min_missing_share: float = typer.Option(0.1, help="Минимальная доля пропусков для флага проблемной колонки."),
) -> None:
    """
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        if quality_flags['has_suspicious_id_duplicates']:
            f.write(f"- Подозрительные дубликаты ID: **{', '.join(quality_flags['suspicious_id_duplicates'])}**\n")
        if quality_flags['has_many_zero_values']:
            f.write(f"- Колонки с множеством нулей: **{', '.join(quality_flags['zero_heavy_columns'])}**\n")
        if quality_flags['has_high_cardinality']:
            f.write(f"- Высокая кардинальность: **{', '.join(quality_flags['high_cardinality_columns'])}**\n")
        f.write("\n")
