...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # Подозрительные дубликаты ID (колонки с "_id" в названии)
    suspicious_id_duplicates = []
    for col in summary.columns:
        if "_id" in col.name.lower() and df[col.name].duplicated().any():
    ...
    # Много нулевых значений (более 50% нулей для числовых колонок)
    zero_heavy_columns = []
    for col in summary.columns:
    ...
    # Высокая кардинальность (уникальных значений больше 80% от общего числа строк)
    high_cardinality_columns = []
    for col in summary.columns:
        if col.unique / summary.n_rows > 0.8:
            high_cardinality_columns.append(col.name)
    flags["has_high_cardinality"] = len(high_cardinality_columns) > 0
