...
### 5. `POST /analyze` – получение флагов качества по CSV-файлу

Дополнительный эндпоинт, реализованный в рамках HW04. Принимает CSV-файл и возвращает только флаги качества и базовую информацию о датасете (упрощенная версия `/quality-from-csv` для быстрой проверки).
...
Этот эндпоинт использует доработки из HW03, включая новые эвристики качества данных (например, проверка на константные колонки, высокую кардинальность категориальных признаков и т.д.).
