...
# ---------- Твой личный вклад: эндпоинт для анализа флагов ----------


@app.post(
    "/analyze",
    tags=["analysis"],
    summary="Получение списка флагов качества для CSV-файла",
)
async def analyze(file: UploadFile = File(...)) -> dict[str, Any]:
    """
    Принимает CSV-файл и возвращает только флаги качества и базовую информацию.
    Это упрощенная версия /quality-from-csv для быстрой проверки.
    """
    if file.content_type not in ("text/csv", "application/vnd.ms-excel", "application/octet-stream"):
